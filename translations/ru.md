...
- [Законы](#законы)
    ...
    - [Закон Деметры / Принцип наименьшего знания](#закон-деметры--принцип-наименьшего-знания)
...
Закон Деметера, также известный как «Принцип наименьшего знания», — это принцип проектирования программного обеспечения, особенно актуальный в объектно-ориентированных языках.

Закон гласит, что программный модуль должен взаимодействовать только со своими непосредственными соседями. Если объект `A` содержит ссылку на объект `B`, то он может вызывать его методы, но если `B` содержит ссылку на `C`, то объект `A` не должен вызывать методы `C` напрямую. Таким образом, если `C` имеет метод `doThing()`, `A` не должен вызывать его напрямую: `B.getC().doThis()`.

Соблюдение этого принципа ограничивает область изменений, делая их проще и безопаснее в будущем.
